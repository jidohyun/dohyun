...
- **CI publish workflow is now idempotent.** Sub-package and main
  publish steps skip versions that already exist on the npm registry
  (checked via `npm view`), so re-runs or partial failures no longer
  conflict. First end-to-end release driven purely by `git push
  --follow-tags`.
