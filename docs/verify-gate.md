...
| `@verify:grep(pattern)` | any file contains the literal `pattern` (skips `node_modules`, `dist`, `.git`, `.dohyun`) |
...
`DOHYUN_SKIP_VERIFY=1 dohyun dod check "..."` bypasses the gate.
A WARN entry lands in `.dohyun/logs/log.md` regardless — the audit
